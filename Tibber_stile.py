...
# Pfade zu Waveshare-E-Paper-Treiber
...
def update_price_cache(price_data):
    ...
    if not cached or cached.get('date') != today_str:
        ...
        save_cache({ "date": today_str, "data": price_data['today'] },
                   CACHE_FILE_TODAY)
...
def filter_yesterday_consumption(cons_data):
    ...
    for rec in cons_data:
        try:
            dt = datetime.datetime.fromisoformat(rec['from']) \
...
def draw_dashed_line(draw, x1, y1, x2, y2,
...
def draw_two_day_chart(draw,
    ...
    while yv <= vmax:
        ...
        draw.text((X0-45, y-7),
                  f"{yv/100:.2f}", font=fonts['small'], fill=0)
        yv += step
    draw.text((X0-45, Y0-20),
              "Preis (ct/kWh)", font=fonts['small'], fill=0)

    # linkes Panel: Preis‐Stufen
    times_l = [
        datetime.datetime.fromisoformat(s['startsAt'])
        .astimezone(local_tz)
        for s in left_data
    ]
    nL = len(times_l)
    ...
    # Verbrauchskurve im Historical-Mode
    if mode == 'historical':
    ...
    # annotate two highs + one low of current day
    if vals_r:
        ...
        for idx, offset in ((idx_min,40),(idx_high1,15),(idx_high2,15)):
            ...
            draw.text((x-15, y-offset),
                      txt, font=fonts['small'], fill=0)
