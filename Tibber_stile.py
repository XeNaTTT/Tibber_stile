...
def draw_two_day_chart(d, left, right, fonts, mode, area, pv_y=None, pv_t=None):
    ...
    if pv_y is not None and pv_t is not None:
        max_y = pv_y.max(skipna=True) or 0
        max_t = pv_t.max(skipna=True) or 0
        pv_max = max(max_y, max_t, 0)
        if pv_max > 0:
            pv_max += 20
            sy_v = H / pv_max
        else:
    ...
    if pv_y is not None:
        pts = []
        for i, val in enumerate(pv_y.fillna(0).tolist()):
    ...
    if pv_t is not None:
        pts = []
        for i, val in enumerate(pv_t.fillna(0).tolist()):
...
# draw_subtitle_labels und draw_info_box bleiben unverändert
...
def load_pv_hourly(date):
    ...
    return df['dtu_power'].resample('H').mean().fillna(0)
...
def main():
    ...
    pv_y  = load_pv_hourly(yest)
    pv_t  = load_pv_hourly(today)
    upper = (0, 0, epd.width, epd.height//2)
    draw_two_day_chart(
        d, left_price, right_price, fonts, 'historical', area=upper,
        pv_y=pv_y, pv_t=pv_t
    )
