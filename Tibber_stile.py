...
# ---- Chart-Helfer mit PV-Overlay & Labels ----
...
def draw_two_day_chart(d, left, right, fonts, mode, area, pv_y=None, pv_t=None):
    ...
    if sy_v:
        ...
        idx = pv_y.values.argmax(); xm,ym = pts[idx]
        d.text((xm-15, ym-15), f"{int(pv_y[idx])}W", font=fonts['small'], fill=0)
    ...
    for i in range(nR-1):
        ...
        d.line((x1,y1,x2,y1), fill=0, width=2); d.line((x2,y1,x2,y2), fill=0, width=2)
        mx = (x1 + x2)/2
        d.text((mx-12, y1-12), f"{vals_r[i]/100:.2f}", font=fonts['small'], fill=0)
    # PV overlay heute + Peak
    if sy_v:
        pts = [(xR[i], Y1-int(v*sy_v)) for i,v in enumerate(pv_t.tolist())]
        for a,b in zip(pts,pts[1:]): draw_dashed_line(d,a[0],a[1],b[0],b[1], dash_length=2, gap_length=2)
        idx = pv_t.values.argmax(); xm,ym = pts[idx]
        d.text((xm-15, ym-15), f"{int(pv_t[idx])}W", font=fonts['small'], fill=0)
...
def draw_subtitle_labels(d, fonts, mode):
    ...
    X0,X1 = 60, epd7in5_V2.EPD().width; PW=(X1-X0)/2; y= int(epd7in5_V2.EPD().height*0.6) + 5
...
def draw_info_box(d, info, fonts):
    X0,X1 = 60, epd7in5_V2.EPD().width; y= int(epd7in5_V2.EPD().height*0.6) + 25
    bf = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",12)
    infos = [
