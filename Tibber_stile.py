...
def prepare_data(pd):
    ...
    cur_dt=datetime.datetime.fromisoformat(pd['current']['startsAt'])\
               .astimezone(local_tz)
...
def draw_two_day_chart(d, left, lt, right, rt, fonts, mode, area):
    X0,Y0,X1,Y1=area;W,X=W,X1-X0;H,Y=Y1-Y0;PW=W/2
...
def draw_two_day_pv(d,fonts,area):
    ...
    def load_day(dt):
        st=int(datetime.datetime.combine(dt,datetime.time.min).timestamp())
        en=int(datetime.datetime.combine(dt,datetime.time.max).timestamp())
        conn=sqlite3.connect(DB_FILE)
        df=pd.read_sql_query(
          "SELECT ts,pv1_power,pv2_power,dtu_power FROM pv_log WHERE ts BETWEEN ? AND ? ORDER BY ts",
          conn,params=(st,en))
        conn.close()
        df['ts']=pd.to_datetime(df['ts'],unit='s',errors='coerce')
        df.set_index('ts',inplace=True)
        df=df.resample('15T').mean().fillna(0)
        # tz-aware index für Vergleich
        df.index=df.index.tz_localize(local_tz)
        return df

    today=datetime.date.today();yesterday=today-datetime.timedelta(days=1)
    df_y,df_t=load_day(yesterday),load_day(today)

    # auf letzte Viertelstunde begrenzen
    now=datetime.datetime.now(local_tz)
    delta=datetime.timedelta(minutes=now.minute%15,seconds=now.second,microseconds=now.microsecond)
    now_floor=now-delta
    ...
    for idx,df in enumerate([df_y,df_t]):
        ox=X0+idx*PW;n=len(df)
        for series,(dash,wd) in [('pv1_power',(None,2)),('pv2_power',(2,1)),('dtu_power',(None,1))]:
            pts=[]
            ...
            for (x1,y1),(x2,y2) in zip(pts,pts[1:]):
                if dash: draw_dashed_line(d,x1,y1,x2,y2,dash_length=dash,gap_length=dash,fill=0,width=wd)
                else:    d.line((x1,y1,x2,y2),fill=0,width=wd)
        ...
        for v in [0,vmax/2,vmax]:
            ...
            lx=X0-45 if idx==0 else X0+PW-45;d.text((lx,y-7),f"{int(v)}W",font=fonts["small"],fill=0)
...
def main():
    ...
    price_info=get_price_data();update_price_cache(price_info)
    cy=get_cached_yesterday();info=prepare_data(price_info)
    left=cy.get('data',[]); right=price_info['today']
