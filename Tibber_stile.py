...
def draw_two_day_chart(draw, left_data, right_data, fonts, mode):
    # Chart-Bereich
    X0,X1 = 60,800
    Y0,Y1 = 50,400
    W,H   = X1-X0, Y1-Y0
    ...
    step = 5
    current = math.floor(vmin/step)*step
    while current <= vmax:
        y = Y1-(current-vmin)*sy
        draw.line((X0-5,y, X0,y), fill=0)
        draw.line((X1,y, X1+5,y), fill=0)
        draw.text((X0-45,y-7), f"{current/100:.2f}", font=fonts['small'], fill=0)
        current += step
    draw.text((X0-45,Y0-20),"Preis (ct/kWh)", font=fonts['small'], fill=0)

    # Linke Preisstufenlinie
    for i in range(nL-1):
...
def main():
    ...
    if pd['tomorrow'] and pd['tomorrow'][0]['total'] is not None:
        ...
        mode = 'historical'
        # Preise gestern + Verbrauchfelder
        old = (get_cached_yesterday() or {}).get('data', [])
        left_data = [{"startsAt": s.get('startsAt', s.get('from')),
                      "total":    s.get('total',0),
                      "consumption": s.get('consumption',0)} for s in old]
        right_data = pd['today']

    draw_two_day_chart(draw, left_data, right_data, fonts, mode)
