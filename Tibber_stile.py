...
def draw_two_day_chart(draw, left_data, left_type, right_data, right_type, fonts, mode):
    ...
    if mode=='historical':
        ...
        if cons_nodes and len(cons_nodes)==len(times_l):
            ...
            for i in range(len(y_cons)-1):
                draw_dashed_line(draw, xL[i], y_cons[i], xL[i+1], y_cons[i+1], fill=0, width=1, dash_length=4, gap_length=4)

    for i,dt in enumerate(times_l):
        if i%2==0:
            draw.text((xL[i],Y1+5), dt.strftime("%Hh"), font=fonts['small'], fill=0)

    # rechtes Panel: Preisdaten (aktueller Tag)
    ...
    # Uhrzeit über dem Tiefstpreis des aktuellen Tages anzeigen
    if vals_r:
        minval = min(vals_r)
        idx_min = vals_r.index(minval)
        x_min = xR[idx_min]
        y_min = Y1 - (vals_r[idx_min]-vmin)*sy
        dt_min = times_r[idx_min].strftime("%H:%M")
        draw.text((x_min-15, y_min-40), dt_min, font=fonts['small'], fill=0)
