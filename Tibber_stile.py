...
# ---- Tibber-Preis-Cache und Abfrage ----
...
def prepare_data(pd):
    ...
    all_slots = [
        (datetime.datetime.fromisoformat(s['startsAt']).astimezone(local_tz), s['total'] * 100)
...
def draw_two_day_chart(d, left_data, lt, right_data, rt, fonts, mode, area=None):
    # area=(x0,y0,x1,y1) oder default
    if area:
        X0, Y0, X1, Y1 = area
    else:
        X0, X1 = 60, 800
...
# ---- PV Stacked-Area Chart (unterer Bereich) ----
...
def draw_pv_stacked(d, fonts, y0, y1, width):
    ...
    # X-axis ticks each hour
    for ts in df.index:
        if ts.minute == 0:
            idx = df.index.get_loc(ts)
            x = int(idx * width/(len(df)-1))
            d.line((x, y1, x, y1+4), fill=0)
            d.text((x-15, y1+6), ts.strftime('%H:%M'), font=fonts['small'], fill=0)
    # Y-axis labels
