...
# Pandas for PV series alignment
...
def prepare_data(pd):
    ...
    slots      = [(datetime.datetime.fromisoformat(s['startsAt']).astimezone(local_tz), s['total']*100) for s in pd['today'] + pd['tomorrow']]
...
def get_pv_series(slots):
    ...
    # Resample 15T, pad forward
    df = df.resample('15T').mean().fillna(method='pad').fillna(0)
    # Build series aligned to slot start times
    vals = []
    for s in slots:
        dt = datetime.datetime.fromisoformat(s['startsAt']).astimezone(local_tz).replace(tzinfo=None)
        try:
...
def draw_two_day_chart(d, left, right, fonts, mode, area, pv_y=None, pv_t=None):
    ...
    if pv_y is not None:
        pts=[]
        for i,v in enumerate(pv_y): pts.append((xL[i], Y1-int(v*sy_v)))
