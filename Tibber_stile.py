...
# ---- Chart-Helfer mit PV-Overlay & Peak-Labels ----
def draw_dashed_line(d, x1,y1,x2,y2, **kw):
    ...
    for i in range(int(dist/step)+1):
        s=e=i*step; e=min(s+dl,dist)
        xa,ya=x1+dx*s/dist, y1+dy*s/dist
        xb,yb=x1+dx*e/dist, y1+dy*e/dist
...
def draw_two_day_chart(d, left, right, fonts, mode, area, pv_y=None, pv_t=None):
    # Innenrand 10px
    X0,Y0,X1,Y1 = area; pad=10
    X0+=pad; Y0+=pad; X1-=pad; Y1-=pad
    W=X1-X0; H=Y1-Y0; PW=W/2

    # Preis-Skala
    vals_l=[s['total']*100 for s in left]; vals_r=[s['total']*100 for s in right]
    allp=vals_l+vals_r
    ...
    # Tages-Tief & Hoch markieren
    if vals_l:
        idx_min=vals_l.index(min(vals_l)); idx_max=vals_l.index(max(vals_l))
        xm_min,ym_min=xL[idx_min],Y1-(vals_l[idx_min]-vmin_p)*sy_p
        xm_max,ym_max=xL[idx_max],Y1-(vals_l[idx_max]-vmin_p)*sy_p
        d.text((xm_min-15,ym_min+5),f"{min(vals_l)/100:.2f}",font=fonts['small'],fill=0)
        d.text((xm_max-15,ym_max-15),f"{max(vals_l)/100:.2f}",font=fonts['small'],fill=0)
    # PV gestern bis Ende Daten
    if sy_v:
        pts=[]
        ...
        # Peak
        pm_idx=pv_y.values.argmax(); xpm,ypm=pts[pm_idx]
        d.text((xpm-15,ypm-15),f"{int(pv_y[pm_idx])}W",font=fonts['small'],fill=0)
    # X-Ticks
    ...
    for i in range(nR-1):
        ...
        d.line((x1,y1,x2,y1),fill=0,width=2); d.line((x2,y1,x2,y2),fill=0,width=2)
    # Tages-Tief & Hoch heute
    if vals_r:
        idx_min=vals_r.index(min(vals_r)); idx_max=vals_r.index(max(vals_r))
        xm_min,ym_min=xR[idx_min],Y1-(vals_r[idx_min]-vmin_p)*sy_p
        xm_max,ym_max=xR[idx_max],Y1-(vals_r[idx_max]-vmin_p)*sy_p
        d.text((xm_min-15,ym_min+5),f"{min(vals_r)/100:.2f}",font=fonts['small'],fill=0)
        d.text((xm_max-15,ym_max-15),f"{max(vals_r)/100:.2f}",font=fonts['small'],fill=0)
    # PV heute nur bis jetzt
    if sy_v:
        now=datetime.datetime.now(local_tz)
        # Schnittstelle
        cut=[i for i,t in enumerate(times_r) if t>now]
        end = cut[0] if cut else len(times_r)
        pts=[]
        for i,v in enumerate(pv_t.tolist()[:end]): pts.append((xR[i],Y1-int(v*sy_v)))
        for a,b in zip(pts,pts[1:]): draw_dashed_line(d,a[0],a[1],b[0],b[1],dash_length=2,gap_length=2)
        pm_idx=pv_t[:end].values.argmax(); xpm,ypm=pts[pm_idx]
        d.text((xpm-15,ypm-15),f"{int(pv_t[pm_idx])}W",font=fonts['small'],fill=0)
    # X-Ticks
...
def draw_subtitle_labels(d, fonts, mode):
    ...
    width=epd7in5_V2.EPD().width; height=epd7in5_V2.EPD().height
    X0=60; PW=(width-X0)/2; y=int(height*0.6)+5
...
def main():
    ...
    # PV-Serien
    pv_y=get_pv_series(left_price); pv_t=get_pv_series(right_price)
    print('PV gestern:', pv_y.tolist())
    print('PV heute: ', pv_t.tolist())

    # Chart auf 60% Höhe
    upper=(int(epd.width*0.05), 0, int(epd.width*0.95), int(epd.height*0.8))  # chart uses 90% width
