...
def draw_subtitle_labels(d,fonts,mode):
    # Dein bisheriger Code für Untertitel einfügen
    pass

def draw_info_box(d,data,fonts):
    # Dein bisheriger Code für Info-Kasten einfügen
    pass
...
def draw_pv_lines(d, fonts, y0, y1, width):
    ...
    # Linien zeichnen
    for i in range(n-1):
        d.line((xs[i],ys1[i],xs[i+1],ys1[i+1]), fill=0, width=2)      # PV1
        draw_dashed_line(d, xs[i],ys2[i], xs[i+1],ys2[i+1],
                         dash_length=2, gap_length=2, fill=0, width=1)  # PV2
        d.line((xs[i],yst[i],xs[i+1],yst[i+1]), fill=0, width=1)      # Total

    # Achsen X (je Stunde) und Y
    for ts in df.index:
        if ts.minute==0:
...
def main():
    ...
    # 2) PV-Linien-Chart unten (Hälfte)
