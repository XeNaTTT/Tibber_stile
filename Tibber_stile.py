...
def prepare_data(pi):
    ...
    if future:
        ...
        hours,fv = 0,0
    return {"current_price":cur_price,"lowest_today":lowest,
            "highest_today":highest,"hours_to_lowest":hours,
...
def draw_two_day_chart(d, left, right, fonts, subtitles, area, pv_y=None, pv_t=None, label_min_max=False):
    X0,Y0,X1,Y1 = area; W,H = X1-X0, Y1-Y0; PW=W/2
    def ext(slots):
        ts,ps=[],[]
        for s in slots:
            ...
            except: continue; ts.append(dt); ps.append(p)
        return ts,ps
    ...
    while yv<=vmax:
        y=Y1-(yv-vmin)*sy; d.line((X0-5,y,X0,y),fill=0); d.line((X1,y,X1+5,y),fill=0)
        d.text((X0-45,y-7),f"{yv/100:.2f}",font=fonts['small'],fill=0); yv+=step
    ...
    def panel(ts,ps,pv,x0):
        ...
        if syv and pv is not None:
            ...
            if valid:
                imax=max(valid,key=lambda i:pv.iloc[i])
                xm,ym=pts[imax]; d.text((xm-15,ym-15),f"{int(pv.iloc[imax])}W",font=fonts['small'],fill=0)
        # Zeitachse
...
def main():
    ...
    # Infobox oben
    info_y=20; img=Image.new('1',(w,h),255); d=ImageDraw.Draw(img)
    fonts={'small':ImageFont.load_default(),'info_font':ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",14)}
    info=prepare_data(pi); draw_info_box(d,info,fonts,info_y)
    # Chartbereich
    area=(mx, info_y+20, w-mx, h-30)
    if tm:
        ls,rs=td,tm; pv_l,pv_r=get_pv_series(td),None
        subs=("Preis & PV heute","Preis & PV morgen"); lm=True
    else:
        ls,rs=yest,td; pv_l,pv_r=get_pv_series(yest),get_pv_series(td)
        subs=("Preise & PV gestern","Preis & PV heute"); lm=False
    draw_two_day_chart(d,ls,rs,fonts,subs,area,pv_y=pv_l,pv_t=pv_r,label_min_max=lm)
    # Footer
...
if __name__=="__main__":
    try: main()
    except: logging.exception("Fehler im Hauptprogramm")
