...
def draw_two_day_pv(d, fonts, area):
    ...
    # Maskiere alle heute-Werte nach der letzten vollen Viertelstunde
    ...
    for idx, df in enumerate([df_y, df_t]):
        ...
        for series, (dash, width) in [
            ...
            for i, val in enumerate(df[series].tolist()):
                if math.isnan(val):
                    # nach now_floor keine Linie
                    break
                x = ox + (i*(PW/(n-1)) if n>1 else PW/2)
                y = Y1 - int((val/vmax)*H)
                pts.append((x,y))
            # zeichne segmentweise
            for a,b in zip(pts, pts[1:]):
                if dash:
                    draw_dashed_line(d, a[0],a[1], b[0],b[1],
                                     dash_length=dash, gap_length=dash,
                                     fill=0, width=width)
                else:
                    d.line((a,b), fill=0, width=width)
...
def main():
    ...
    # Preis-Chart: immer gestern vs heute
    ...
    left = cy.get('data', [])
    right = price_info['today']

    upper = (0, 0, epd.width, epd.height//2)
    draw_two_day_chart(d, left, "price", right, "price",
